...
class Chunker:
    def __init__(self, logger_instance=None,  # Allow passing logger
    ...
    def _create_rag_item(self, enriched_item: EnrichedItem, chunk_text: str,
        ...
        # Keyphrases for the chunk:
        # For simplicity, we'll pass all parent keyphrases to each chunk.
        # More advanced: extract keyphrases specific to the chunk_text itself.
        ...
        return RAGOutputItem(
            ...
            keyphrases=chunk_keyphrases,  # <-- Propagate keyphrases
            # Entities specific to this chunk (could be refined if enricher does per-chunk NER)
            # For now, use element_specific_entities if provided (e.g., from a structured block that had its own NER)
            # otherwise, fall back to a slice of parent's overall_entities as a placeholder.
    ...
    def _chunk_single_content_piece(self, content: str, enriched_item: EnrichedItem,
                                    ...
                                    # Allow passing entities specific to this piece of content
        ...
        while start < text_length:
            ...
            # Ensure overlap doesn't push start too far back if remaining text is short
            start = max(0, end - self.overlap_size)
            # Safety break for very short overlaps or small max_chunk_size causing infinite loop
            if len(text_chunks_content) > text_length / (
            self.min_chunk_size / 2 if self.min_chunk_size > 0 else 10) + 10:  # Heuristic
                self.logger.warning(
                    f"Potential infinite loop in chunking for {enriched_item.source_url}, parent_type {parent_type}. Breaking.")
                break
        ...
        for i, chunk_content_str in enumerate(text_chunks_content):
            ...
            if len(stripped_chunk) < self.min_chunk_size:
                self.logger.debug(
                    f"Skipping chunk {i} (type: {parent_type}) because its stripped length ({len(stripped_chunk)}) is less than min_chunk_size ({self.min_chunk_size}).")
                continue

            # For element_specific_entities, if NER was run on `content` before chunking,
            # one would need to map those entities to the current chunk.
            # For now, if `element_entities` are provided for the whole `content` piece,
            # we can pass them along, but they won't be chunk-specific yet.
            # A more robust solution is to run NER on each `stripped_chunk` post-splitting.
            # This is a placeholder for that more advanced logic.
            chunk_specific_entities = []  # Default to empty
            # if element_entities: # If entities for the whole block were passed
            # Logic to find which of element_entities fall within this chunk_content_str
            # This is non-trivial and requires character offset mapping.
            # For now, we'll pass all element_entities, or rely on the RAGOutputItem's fallback.
            # chunk_specific_entities = element_entities # Or a subset

            rag_item = self._create_rag_item(
                enriched_item, stripped_chunk,  # Use stripped_chunk
                ...
                element_specific_entities=chunk_specific_entities  # Pass chunk-specific entities if available
            )
    ...
    def chunk_item(self, enriched_item: EnrichedItem) -> List[RAGOutputItem]:
        ...
        if enriched_item.primary_text_content:
            ...
            text_chunks = self._chunk_single_content_piece(
                ...
                # Pass overall_entities for the primary text, _create_rag_item might use a slice.
        ...
        for idx, structured_element in enumerate(enriched_item.enriched_structured_elements):
            # Determine content based on type (e.g. figure with caption)
            ...
            if element_type == 'semantic_figure_with_caption':
                fig_text = structured_element.get('figure_content', '')
                cap_text = structured_element.get('caption_content', '')
                # Combine them for chunking, perhaps with a separator or distinct labeling if desired
                # For now, just concatenating. Could be "Figure: [fig_text]\nCaption: [cap_text]"
                element_content_to_chunk = f"{fig_text} {cap_text}".strip()
            else:
                element_content_to_chunk = structured_element.get('content')

            element_lang_hint = structured_element.get('language')
            # Entities for this specific structured element (if NER was run per element)
            # For now, assuming structured_element dict might have an 'entities' key.
            ...
            if element_content_to_chunk:
                element_chunks = self._chunk_single_content_piece(
                    element_content_to_chunk,
                    enriched_item,
                    parent_type=element_type,
                    parent_element_idx=idx,
                    element_lang=element_lang_hint,
                    element_entities=element_specific_entities  # Pass entities for this block
                )
